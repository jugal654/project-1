...
class RateCard(BoxLayout):
    ...
    def update_rate(self, new_rate_str):
        # Extract numeric value from the rate string (without currency symbol)
        try:
            # Remove any non-digit characters except commas
            clean_str = ''.join(c for c in new_rate_str if c.isdigit() or c == ',')
            clean_str = clean_str.replace(',', '')
            
            if clean_str:  # Only proceed if we have digits
                new_rate = int(clean_str)
                ...
                formatted_rate = "{:,}".format(new_rate)
                self.rate_label.text = formatted_rate
        except:
            # If parsing fails, just update the text (removing ₹ if present)
            self.rate_label.text = new_rate_str.replace('₹', '').strip()
        ...
        try:
            ...
            if info:
                ...
                if match:
                    ...
                    adjusted_gold_rate = gold_rate - 2000
        ...
        try:
            ...
            if info:
                ...
                if match:
                    ...
                    adjusted_silver_rate = silver_rate * 100   # Adjusted rate calculation
                    adjusted_silver_rate_gst = silver_rate * 100 + 2000  # GST rate calculation
    ...
    def simulate_fluctuation(self):
        ...
        try:
            ...
            if self.base_gold_rate > 0:
                gold_fluctuation = random.randint(+200, +500)
            ...
            if self.base_silver_rate > 0:
                ...
                adjusted_fluctuation = random.randint(-100, 200)
